...
public class ServletRegister extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (runner == null) {
			error(req, "Corredor no encontrado.");
			forward(req, res);
			return;
		}
		if (stage == null) {
			error(req, "Etapa no encontrada.");
			forward(req, res);
			return;
		}
		if (stage.isCompleted()) {
			error(req, "La etapa ya está completada.");
			forward(req, res);
			return;
		}
		if (runner.getIdStage() != null && runner.getIdStage() == stageId) {
			error(req, "El corredor ya está inscrito en esa etapa.");
			forward(req, res);
			return;
		}
		...
		try (Connection conn = DatabaseConnection.getConnection();
			...
			req.setAttribute("message",
					"¡Corredor #" + bib + " inscrito en \"" + stage.getName() + "\" correctamente!");
