...
public class PersonDAO {
	...
	public Person getPersonByID(int id) {
		try (Connection conn = DatabaseConnection.getConnection();
				PreparedStatement ps = conn
						.prepareStatement("SELECT id, name, age, country, dni FROM person WHERE id = ?")) {
			ps.setInt(1, id);
	...
	public int insertPersonAndGetId(Person p, Connection conn) throws SQLException {
		try (PreparedStatement ps = conn.prepareStatement(
				"INSERT INTO person (name, age, country, dni) VALUES (?,?,?,?)", Statement.RETURN_GENERATED_KEYS)) {
	...
	private Person mapRow(ResultSet rs) throws SQLException {
		return new Person(rs.getInt("id"), rs.getString("name"), rs.getInt("age"), rs.getString("country"),
				rs.getString("dni"));
	}
}
