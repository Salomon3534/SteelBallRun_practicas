...
public class UserDAO {
	...
	public User findByCredentials(String username, String passkeyHash) {
		try (Connection conn = DatabaseConnection.getConnection();
				PreparedStatement ps = conn.prepareStatement(SELECT + " WHERE username = ? AND passkey = ?")) {
	...
	public int insertAndGetId(User u, Connection conn) throws SQLException {
		...
		try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
			...
			if (u.getRunnerId() != null)
				ps.setInt(4, u.getRunnerId());
			else
	...
	private User mapRow(ResultSet rs) throws SQLException {
		...
		return new User(rs.getInt("id"), rs.getString("username"), rs.getString("passkey"), rs.getString("role"),
				runnerId);
	}
}
